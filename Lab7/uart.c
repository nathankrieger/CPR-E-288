...
void uart_init(void){
  ...
  //calculate baud rate
  uint16_t iBRD = 8;//SYSCLOCK / (CLCDIV * BAUDRATE); //use equations (8)
  uint16_t fBRD = 44;//(int)((SYSCLOCK / (CLCDIV * BAUDRATE)) % iBRD * 64 + .5); //use equations (44)

  //turn off UART1 while setting it up
  UART1_CTL_R &= 0xFCFE;//sets 8 and 9 to 0
